...
# Webpage content extraction tab
with tab2:
    url_input = st.text_input("Paste a webpage link to extract content:")

    if url_input:
        try:
            res = requests.get(url_input, timeout=10, verify=False)  # Bypass SSL verification
            ...
            if full_text.strip():
                ...
                st.warning("⚠️ No readable paragraph text found on the page.")
        except Exception as e:
            st.error(f"❌ Error reading link:\n{e}")
